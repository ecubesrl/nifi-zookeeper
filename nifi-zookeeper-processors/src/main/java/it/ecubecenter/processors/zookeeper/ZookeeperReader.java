...
@WritesAttributes({@WritesAttribute(attribute="Output attribute", description="The value read is put in the attribute specified in the property Output attribute.")})
public class ZookeeperReader extends AbstractZookeeperProcessor {
    ...
    public static final PropertyDescriptor OUTPUT_ATTRIBUTE_NAME = new PropertyDescriptor
            .Builder().name("Output attribute")
            .description("The name of the attribute in which the value read is written.")
            .required(true)
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .build();
    ...
    protected void init(final ProcessorInitializationContext context) {
        ...
        descriptors.add(OUTPUT_ATTRIBUTE_NAME);
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        while(trials>0) {
            try {
                ...
                getLogger().warn("Error while reading from Zookeeper", e);
            }
            trials--;
        }
        if(trials == 0){
